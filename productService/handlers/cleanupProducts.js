...
exports.cleanupProducts = async () => {
    try {
        ...
        //Calculate the timestamp for 1 hour ago(to filter outdated Products)
        ...
        const scanCommand = new ScanCommand({
            ...
            FilterExpression: "createdAt < :oneHourAgo AND attribute_not_exists(imageUrl)", //Filter Products older than one hour and without imageUrl
            ExpressionAttributeValues: {
                ":oneHourAgo": {S: oneHourAgo}, //Bind the Timestamp for filtering
            },
        });

        //Execute the scan command to get outdated Products
        ...
        if (!Items || Items.length === 0) {
            return {
                ...
                body: JSON.stringify({
                    message: "No outdated Products found", //Success message
                }),
            };
        }

        //Initialize a counter to keep track of deleted Products
        let deletedCount = 0;
        ...
        const snsMessage = `Cleanup completed: ${deletedCount} outdated Products deleted.`;

        await snsClient.send(new PublishCommand({
            TopicArn: snsTopicARN, //SNS topic ARN to publish the message
            Message: snsMessage, //Message to be sent
            Subject: "Product Cleanup Notification", //Subject of the SNS message
        }));

        //Return a success response indicating the number of deleted Products
        return {
            statusCode: 200, //HTTP status code for successful request
            body: JSON.stringify({
                message: `${deletedCount} outdated Products deleted successfully`, //Success message with count of deleted Products
            }),
        };
        ...
        return {
            ...
            body: JSON.stringify({
                message: "An error occurred while cleaning up Products", //Error message
